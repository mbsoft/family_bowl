/**
 * Script to import 2021 archive data from Excel file
 * Run with: node scripts/import-2021-archive.js
 * 
 * Excel structure (1-based row numbers):
 * - Row 2 (index 1): Usernames starting at column E (index 4)
 * - Row 3 (index 2): Point totals starting at column E (index 4), "Result" in column D
 * - Row 4+ (index 3+): Bet data
 *   - Column A: Bet number
 *   - Column B: Question
 *   - Column C: Type
 *   - Column D: Result
...
const EXCEL_FILE = path.join(__dirname, '..', 'Untitled spreadsheet (3).xlsx');
const YEAR = 2021;
...
async function importArchive() {
  ...
  try {
    ...
    // Extract usernames from row 2 (index 1), starting at column E (index 4)
    const usernames = [];
    for (let i = 4; i < jsonData[1].length; i++) {
      const username = String(jsonData[1][i] || '').trim();
    ...
    // Extract point totals from row 3 (index 2) for verification
    const expectedPoints = {};
    for (let i = 0; i < usernames.length; i++) {
      const points = jsonData[2][4 + i];
    ...
    // Process bet rows (starting at row 4, index 3)
    // Questions are on odd rows (3, 5, 7, etc.), skip even rows (4, 6, 8, etc.) which have correct/incorrect
    for (let i = 3; i < jsonData.length; i += 2) {
      ...
      let result = String(row[3] || '').trim(); // Result is in column D (index 3)
      ...
      if (!result || result === '' || result === ' ') {
        if (question.toLowerCase().includes('tie breaker')) {
          // For 2021, the tiebreaker result was 55 (total points scored in Super Bowl LV)
          // TB 31 + KC 9 = 40 (but this seems wrong, let me check actual)
          // Actually Super Bowl LV: TB 31, KC 9 = 40 total
          // But user said Mary wins, so let me use what's in the sheet
          result = '40';
        }
      }
    ...
    if (topUsers.length === 1) {
      ...
      if (topUsers[0].username === 'Mary') {
        console.log(`   ✅ Expected winner (Mary) matches!`);
      } else {
        console.log(`   ⚠️  Expected winner is Mary, but calculated winner is ${topUsers[0].username}`);
      }
    } else {
      ...
      if (tieBreakerBet && results[tieBreakerBet.id]) {
        ...
        if (!isNaN(tieBreakerResult)) {
          ...
          if (winner === 'Mary') {
            ...
            console.log(`   ⚠️  Expected winner is Mary, but calculated winner is ${winner}`);
          }
        } else {
...
function extractTeamNames(question, type) {
  ...
  if (q.includes('tb') || q.includes('tampa bay')) {
    return { option1: 'TB', option2: 'KC' };
  }
  if (q.includes('kc') || q.includes('kansas city')) {
    return { option1: 'TB', option2: 'KC' };
  }
  
  // Check if type has team names (e.g., "TB/KC")
  if (type.includes('/')) {
