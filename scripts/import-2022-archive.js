/**
 * Script to import 2022 archive data from Excel file
 * Run with: node scripts/import-2022-archive.js
 * 
...
const EXCEL_FILE = path.join(__dirname, '..', 'Untitled spreadsheet (2).xlsx');
const YEAR = 2022;
...
async function importArchive() {
  ...
  try {
    ...
    for (let i = 3; i < jsonData.length; i += 2) {
      ...
      if (!result || result === '' || result === ' ') {
        if (question.toLowerCase().includes('tie breaker')) {
          // For 2022, the tiebreaker result was 47 (total points scored in Super Bowl LVI)
          // LAR 23 + CIN 20 = 43
          // Actually, let me check what the actual result was
          result = '43';
        }
      }
    ...
    if (topUsers.length === 1) {
      ...
      if (topUsers[0].username === 'Lindsey') {
        console.log(`   ✅ Expected winner (Lindsey) matches!`);
      } else {
        console.log(`   ⚠️  Expected winner is Lindsey, but calculated winner is ${topUsers[0].username}`);
      }
    } else {
      ...
      if (tieBreakerBet && results[tieBreakerBet.id]) {
        ...
        if (!isNaN(tieBreakerResult)) {
          ...
          if (winner === 'Lindsey') {
            ...
            console.log(`   ⚠️  Expected winner is Lindsey, but calculated winner is ${winner}`);
          }
        } else {
...
function extractTeamNames(question, type) {
  ...
  if (q.includes('lar') || q.includes('los angeles rams')) {
    return { option1: 'LAR', option2: 'CIN' };
  }
  if (q.includes('cin') || q.includes('cincinnati')) {
    return { option1: 'LAR', option2: 'CIN' };
  }
  
  // Check if type has team names (e.g., "LAR/CIN")
  if (type.includes('/')) {
