/**
 * Script to import 2023 archive data from Excel file
 * Run with: node scripts/import-2023-archive.js
 * 
 * Excel structure:
...
const EXCEL_FILE = path.join(__dirname, '..', 'Untitled spreadsheet.xlsx');
const YEAR = 2023;
    ...
    for (let i = 2; i < jsonData.length; i += 2) {
      ...
      if (!result || result === '' || result === ' ') {
        if (question.toLowerCase().includes('tie breaker')) {
          // For 2023, the tiebreaker result was 73 (total points scored in Super Bowl LVII)
          // KC 38 + PHI 35 = 73
          result = '73';
        }
      }
    ...
    if (topUsers.length === 1) {
      ...
      if (tieBreakerBet && results[tieBreakerBet.id]) {
        ...
        if (!isNaN(tieBreakerResult)) {
          ...
          if (winner === 'Corey') {
            console.log(`   ✅ Expected winner (Corey) matches!`);
          } else {
            console.log(`   ⚠️  Expected winner is Corey, but calculated winner is ${winner}`);
          }
        } else {
...
function extractTeamNames(question, type) {
  ...
  if (q.includes('phl') || q.includes('philadelphia')) {
    return { option1: 'PHL', option2: 'KC' };
  }
  if (q.includes('kc') || q.includes('kansas city')) {
    return { option1: 'PHL', option2: 'KC' };
  }
  
  // Check if type has team names (e.g., "PHL/KC")
  if (type.includes('/')) {
